...
const openDispute = async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      "dispute",
      "Disputa abierta",
      `Se abrió una disputa para "${project.title}".`,
      dispute._id,
    );
...
const getDisputes = async (req, res) => {
  try {
    if (req.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Solo admins pueden ver todas las disputas." });
    }
...
const getDisputeById = async (req, res) => {
  try {
    ...
    if (dispute.project_id && dispute.project_id.on_chain_id) {
      try {
        ...
        onChainData = await contracts.getProject(
          platformPublicKey,
          dispute.project_id.on_chain_id,
        );
      } catch {
        /* ignore */
      }
    ...
    res
      .status(200)
      .json({ success: true, data: { dispute, evidence, onChainData } });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
const resolveDispute = async (req, res) => {
  try {
    if (req.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Solo admins pueden resolver disputas." });
    }
    ...
    if (favorFreelancer === undefined) {
      return res
        .status(400)
        .json({ error: "favorFreelancer (boolean) es requerido." });
    }
    ...
    if (project && project.on_chain_id) {
      try {
        ...
        await contracts.resolveDispute(
          adminKeypair,
          project.on_chain_id,
          favorFreelancer,
        );
    ...
    if (project) {
      ...
      if (escrow && escrow.status === "disputed") {
        const winnerId = favorFreelancer
          ? project.freelancer_id
          : project.recruiter_id;
      ...
      const resultado = favorFreelancer
        ? "a favor del freelancer"
        : "a favor del reclutador";
      await createNotification(
        project.freelancer_id,
        "dispute",
        "Disputa resuelta",
        `La disputa de "${project.title}" fue resuelta ${resultado}.`,
        dispute._id,
      );
      await createNotification(
        project.recruiter_id,
      ...
      // ── Email: dispute resolved (critical — always send to both parties) ──
      const User = require("../models/User");
      const [freelancerUser, recruiterUser] = await Promise.all([
        User.findById(project.freelancer_id).select("email username"),
        User.findById(project.recruiter_id).select("email username"),
      ]);
      for (const recipient of [freelancerUser, recruiterUser]) {
        if (!recipient?.email) continue;
        const html = loadTemplate("dispute-resolved", {
          recipientName: recipient.username || "Usuario",
          projectTitle: project.title,
          resolution: resultado.charAt(0).toUpperCase() + resultado.slice(1),
          ctaUrl: `${FRONTEND_URL}/projects/${project._id}`,
        });
        await sendEmail(
          recipient.email,
          `Resolución de disputa: ${project.title}`,
          html,
        );
      }
    }
    ...
    res
      .status(200)
      .json({ success: true, data: { message: "Disputa resuelta.", dispute } });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
  ...
  try {
    ...
    if (dispute.status === "resolved") {
      return res.status(400).json({
        error: "No se puede agregar evidencia a una disputa resuelta.",
      });
    }
