...
const createEvent = async (req, res) => {
  try {
    if (req.role !== "recruiter" && req.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Solo reclutadores pueden crear eventos." });
    }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "createEvent: secret decryption failed —",
          decErr.message,
          "— skipping escrow transfer",
        );
      }

      const platformPubKey = require("@stellar/stellar-sdk")
        .Keypair.fromSecret(process.env.PLATFORM_SECRET)
        .publicKey();
      ...
      if (rawSecret && rawSecret.startsWith("S")) {
        ...
        console.warn(
          "createEvent: no decrypted secret — skipping XLM escrow (will retry in production)",
        );
      }
    } catch (escrowErr) {
    ...
    res
      .status(201)
      .json({ success: true, data: { stellarTxHash, event: savedEvent } });
  } catch (err) {
  ...
  try {
    ...
    if (event.on_chain_id) {
      try {
        ...
        onChainData = await contracts.getEvent(
          platformPublicKey,
          event.on_chain_id,
        );
      } catch {
        /* ignore si falla la consulta on-chain */
      }
    }
...
const applyToEvent = async (req, res) => {
  try {
    if (req.role !== "freelancer") {
      return res
        .status(403)
        .json({ error: "Solo freelancers pueden aplicar." });
    }
    ...
    if (existing)
      return res.status(400).json({ error: "Ya aplicaste a este evento." });
    ...
    if (event.on_chain_id) {
      try {
        ...
        return res
          .status(500)
          .json({ error: "Error aplicando on-chain: " + contractErr.message });
      }
    }
...
const submitWork = async (req, res) => {
  try {
    if (req.role !== "freelancer") {
      return res
        .status(403)
        .json({ error: "Solo freelancers pueden enviar entregables." });
    }
    ...
    if (!participant)
      return res.status(403).json({ error: "No has aplicado a este evento." });
    ...
    if (event && event.on_chain_id && content) {
      try {
        ...
        entryHash = await contracts.submitEntry(
          freelancerKeypair,
          event.on_chain_id,
          content,
        );
        ...
        return res.status(500).json({
          error: "Error enviando entregable on-chain: " + contractErr.message,
        });
      }
    }
  ...
  try {
    ...
    if (event.recruiter_id.toString() !== req.userId) {
      return res.status(403).json({
        error: "Solo el creador del evento puede seleccionar ganadores.",
      });
    }
    ...
    if (event.on_chain_id && winners && winners.length > 0) {
      try {
        ...
        return res.status(500).json({
          error:
            "Error seleccionando ganadores on-chain: " + contractErr.message,
        });
      }
    }
    ...
    for (const submissionId of submissionIdList) {
      ...
      try {
        ...
        const current = await SearchIndexFreelancers.findOne({
          user_id: submission.freelancer_id,
        });
        if (current) {
          const eventsWon =
            (current.rating > 0 ? Math.round((current.rating - 3) / 0.5) : 0) +
            1;
        ...
        console.error(
          "Error actualizando SearchIndexFreelancers:",
          indexErr.message,
        );
      }
    ...
    if (escrow && winnerUserIds.length > 0) {
      ...
      for (const winnerId of winnerUserIds) {
        ...
        try {
          ...
          console.log(
            `✅ Prize XLM sent to ${winnerWallet.stellar_address}: ${txHash}`,
          );
        } catch (payErr) {
          console.error(
            `❌ Prize XLM transfer failed for ${winnerId}:`,
            payErr.message,
          );
        }
    ...
    res.status(200).json({
      success: true,
      data: { message: "Ganadores seleccionados.", winners: winnerUserIds },
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
const timeoutDistribute = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: { message: "Evento expirado y fondos redistribuidos." },
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
