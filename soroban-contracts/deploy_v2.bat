...
set PASSPHRASE=Test SDF Network ; September 2015
...
for /f "delims=" %%i in ('stellar contract deploy --wasm target/wasm32v1-none/release/reputation_ledger.wasm --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%"') do set REPUTATION_ID=%%i
if "!REPUTATION_ID!"=="" (
...
for /f "delims=" %%i in ('stellar contract deploy --wasm target/wasm32v1-none/release/event_contract.wasm --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%"') do set EVENT_ID=%%i
if "!EVENT_ID!"=="" (
...
for /f "delims=" %%i in ('stellar contract deploy --wasm target/wasm32v1-none/release/project_contract.wasm --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%"') do set PROJECT_ID=%%i
if "!PROJECT_ID!"=="" (
...
for /f "delims=" %%i in ('stellar contract deploy --wasm target/wasm32v1-none/release/wallet_registry.wasm --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%"') do set WALLET_ID=%%i
if "!WALLET_ID!"=="" (
...
REM ── Macro para invoke (evita repetir flags en cada llamada) ─
REM   Uso: call :invoke <CONTRACT_ID> <args...>
REM   No es posible macro real en .bat, los flags se repiten inline por claridad.

REM ── Inicializar contratos ──────────────────────────────────
echo [6/7] Inicializando contratos...

echo    Inicializando ReputationLedger...
stellar contract invoke --id !REPUTATION_ID! --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%" -- initialize --admin !ADMIN_ADDR! --wallet_registry_addr !WALLET_ID!
if %errorlevel% neq 0 (
...
stellar contract invoke --id !EVENT_ID! --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%" -- initialize --admin !ADMIN_ADDR! --token %XLM_TOKEN% --reputation_addr !REPUTATION_ID! --platform_addr !PLATFORM_ADDR! --wallet_registry_addr !WALLET_ID!
if %errorlevel% neq 0 (
...
stellar contract invoke --id !PROJECT_ID! --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%" -- initialize --admin !ADMIN_ADDR! --token %XLM_TOKEN% --reputation_addr !REPUTATION_ID! --platform_addr !PLATFORM_ADDR! --wallet_registry_addr !WALLET_ID!
if %errorlevel% neq 0 (
...
stellar contract invoke --id !WALLET_ID! --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%" -- initialize --admin !ADMIN_ADDR!
if %errorlevel% neq 0 (
...
stellar contract invoke --id !REPUTATION_ID! --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%" -- authorize_contract --contract !EVENT_ID!
if %errorlevel% neq 0 (
...
stellar contract invoke --id !REPUTATION_ID! --source admin --network "%NETWORK%" --network-passphrase "%PASSPHRASE%" --rpc-url "%RPC_URL%" -- authorize_contract --contract !PROJECT_ID!
if %errorlevel% neq 0 (
